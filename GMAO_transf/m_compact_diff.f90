...
!$$$ end documentation block
  ...
  integer(i_kind) :: noq
  integer(i_kind) :: iglobal,itotsub
  integer(i_kind),allocatable,dimension(:):: ltosi
  integer(i_kind),allocatable,dimension(:):: ltosj
  integer(i_kind),allocatable,dimension(:):: ltosi_s
  integer(i_kind),allocatable,dimension(:):: ltosj_s
  real(r_kind),allocatable,dimension(:):: coef
  real(r_kind),allocatable,dimension(:):: coslon
  real(r_kind),allocatable,dimension(:):: sinlon
  
  real(r_kind),parameter :: zero=0.0
  real(r_kind),parameter ::  one=1.0
  real(r_kind),parameter ::  two=2.0
  real(r_kind),parameter :: half=0.5
  real(r_kind),parameter ::   pi=3.141593e+0 ! as in GSI
! real(r_kind),parameter :: pi=acos(-one)
  ...
  interface uv2vordiv; module procedure &
      uv2vordiv_vec_, &
      uv2vordiv_arr_
  end interface 
...
contains
  ...
  subroutine init_compact_diffs(nGlat,nGlon)
...
!$$$
    implicit none
    integer(i_kind),intent(in) :: nGlat,nGlon
  ...
  subroutine create_cdiff_coefs(nGlon,glons)
!$$$  subprogram documentation block
!                .      .    .                                       .
...
!$$$
  implicit none
  integer(i_kind),intent(in) :: nGlon
  real(r_kind),dimension(nGlon),intent(in):: glons

  integer :: i, j, ns, i1

  if(nlon.ne.nGlon)then
    write(6,*)'stop in compact_diff routine'
    stop
  endif
  allocate(coef(3*nlat+4*(2*(noq+5)+1)*(nlat+nlon/2)))
  allocate(ltosi  (nlat*nlon),ltosj  (nlat*nlon),   &
           ltosi_s(nlat*nlon),ltosj_s(nlat*nlon)   )
  allocate(coslon(nlon),sinlon(nlon))
...
!------
  do i=1,nlon
    coslon(i)=cos(glons(i))
    sinlon(i)=sin(glons(i))
  end do
...
! work 1 is u, work2 is v
  ...
  subroutine uv2vordiv_vec_(work1,work2)
!$$$  subprogram documentation block
!                .      .    .                                       .
...
! Declare passed variables
  ...
  real(r_kind),dimension(nlat-1),intent(in):: yor
...
! program history log:
!   2005-05-16  parrish
!   2005-07-20  w. gu   - bug fix in work2 usage and do loop indexing
!
!   input argument list:
!     "b"    - array containing the scalar field
...
!    multiply by cos(lat) ( 1/coef(lcy+iy) )
    do ix=1,nlon
       do iy=1,ny
!       NOTE:  Adjoint of first derivative is its negative
          work2(iy,ix)=work2(iy,ix)/coef(lcy+iy)    !????/check this in particular
       enddo
    enddo
  end if
...
! Declare passed variables
  real(r_kind),dimension(:,:,:),intent(inout):: u,v

! Declare local variables
  integer nGlat,nGlon,nsig
...
! Initialize variables
  ...
  if(isize.ne. nGlat*nGlon)then
    print*,"STOP(uv2vordiv_arr_): isize.ne.nGlat*nGlon"
    stop
  endif
...
!   call u,v --> vor,div routine 
    call uv2vordiv_vec_(work3(1),work4(1))
 ...
 return
end subroutine uv2vordiv_arr_

end module m_compact_diff
