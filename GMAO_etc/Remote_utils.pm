...
# 
# 20190101 Lucchesi  Added SFTP capability to rflist and rget
#
...
# Check SFTP argument
   ...
   if ( exists( $options{"sftp"} ) ) {
      $sftp = $options{"sftp"};
      print "ftp_rflist: SFTP detected in options.  SFTP=$sftp\n";
   } else {
      $sftp = 0;
      print "ftp_rflist: SFTP not detected.  Default set to 0\n";
   }
...
# Get the listing from the remote machine.
   ...
   if ( exists( $options{"sftp"} ) ) {
#     my %args = ( user => "$remote_id", ssh_args => { identity_files => "/home/dao_ops/.ssh/id_rsa.modpdr", protocol=>'2,1' });
#     my $sftp = Net::SFTP::Foreign->new( "${remote_machine}", user => "${remote_id}", ssh_args => { identity_files => [ "/home/dao_ops/.ssh/id_rsa.modpdr" ] } );
      print "Net::SFTP::Foreign->new( ${remote_machine}, user => ${remote_id}, key_path => /home/dao_ops/.ssh/id_rsa.modpdr\n";
      $sftp = Net::SFTP::Foreign->new( "${remote_machine}", user => "${remote_id}", key_path => "/home/dao_ops/.ssh/id_rsa.modpdr" );
      $sftp->die_on_error("Unable to establish SFTP connection");
   }
   else {
      print "Calling FTP->new. remote_machine=$remote_machine\n";
...
# binary files.
   ...
   if ( exists( $options{"sftp"} ) ) {
      @retlist = $sftp->glob($remote_file, names_only => 1 );
      $sftp->die_on_error("Unable to run sftp->glob");
   }
   else {
      $ftp->binary();
...
# If there is login information, use ftp to transfer the file.
        ...
        if ( $remote_machine eq "http" || $remote_machine eq "https" ) {
           ...
           print "Using FTP\n";
           print "$remote_id, $remote_machine, $remote_file,$local_file, %options\n";
...
# Create a command to do the remote copy.
      ...
      $cmd = "$RCP$pflag $remote_id\@$remote_machine:$remote_file $local_file";
      print "cmd =$cmd\n";
      system( $cmd );
      $retcode = $? >> 8;
...
# Check SFTP argument in options hash.

   if ( exists( $options{"sftp"} ) ) {
      $sftp = $options{"sftp"};
      print "SFTP detected in options.  SFTP=$sftp\n";
   } else {
      $sftp = 0;
      print "SFTP not detected.  Default set to 0\n";
   }
...
#    This modules contains the ftp interface.
      ...
      if ( exists( $options{"sftp"} ) ) {
         ...
         $sftp = Net::SFTP::Foreign->new( "${remote_machine}", user => "${remote_id}", key_path => "/home/dao_ops/.ssh/id_rsa.modpdr" );
         $sftp->die_on_error("Unable to establish SFTP connection");
         $sftp->get( $remote_file, $local_file ) or $ftp_ok = 0;
         $sftp->disconnect;
      }
