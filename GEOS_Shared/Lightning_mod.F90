...
! !PARAMETERS:
   ...
   real(kind=REAL64), parameter :: AVOGAD  = 6.0221367d+23 ! Avogadro number (mole^-1)
   real(kind=REAL64), parameter :: CONVFAC = 2.33e-23      ! 14g of N per mole of NO/ 6.02e23 molecules of NO/mole of NO.
   real(kind=REAL64), parameter :: PRODFAC = 1.0e26        ! (joules per flash) * molecules of NO per joule
   real(kind=REAL64), parameter :: Pa2hPa  = 0.01
   integer,parameter  :: hp = KIND( REAL( 0.0, REAL64 ) ) ! HEMCO type
...
! !INPUT PARAMETERS:
      ...
      real(kind=REAL64), intent(in) :: cldmas(:, :)    
      
      !     Desired mean lightning NO production rate (Tg yr-1) (specified in namelist)       	
      real(kind=REAL64), intent(in) :: desired_g_N_prod_rate
      ...
      real(kind=REAL64), intent(inout) :: ratio_local(:,:) 

      real(kind=REAL64), intent(in) :: midLatAdj(:,:) 
      ...
      real(kind=REAL64), intent(in) :: ratio_global             
            
      
! !OUTPUT PARAMETERS:

      !     Total (CG+IC) flash rate (flashes per grid box per s)
      real(kind=REAL64), intent(inout) :: flashrate(:,:)
...
!-------------------------------------------------------------------------
      ...
      ! !Local variables
      real(kind=REAL64), allocatable :: cldmas_local(:,:)
...
!-----------------------------------------------------------------------------
 SUBROUTINE emiss_lightning (i1, i2, j1, j2, k1, k2, minDeepCloudTop, ampFactor, numberNOperFlash, &
  ...
  real(kind=REAL64),  INTENT(IN)  :: flashrate(i1:i2, j1:j2)        ! Flash rate [km^{-2} s^{-1}]
  real(kind=REAL64),  INTENT(IN)  :: dtrn(i1:i2, j1:j2, k1:k2)      ! Detrainment [kg m^{-2} s^{-1}]
  real(kind=REAL64),  INTENT(IN)  :: cellDepth(i1:i2, j1:j2, k1:k2) ! Grid cell depth [m]
  
  real(kind=REAL64), INTENT(OUT)  :: pNOx3D(i1:i2, j1:j2, k1:k2)    ! Lightning NO production rate [m^{-3} s^{-1}]
  real(kind=REAL64), INTENT(OUT)  :: kgNOx3D(i1:i2, j1:j2, k1:k2)   ! NO production rate [kg m^{-3} s^{-1}]

! Local
! -----
  INTEGER :: k
  INTEGER :: status, rc
  real(kind=REAL64), ALLOCATABLE  :: pNOx2D(:,:)                    ! Lightning NO production [molecules NO m^{-2} s^{-1}]
...
! !INPUT PARAMETERS:
   ...
   real(kind=REAL64), intent(in) :: pNOx2D(:,:) 
   real(kind=REAL64), intent(in) :: dtrn(:,:,:) ! Detrainment [kg m^{-2}s^{-1}]
   real(kind=REAL64), intent(in)  :: cellDepth(:,:,:) ! Grid cell depth [m]
   real, intent(in)  :: minDeepCloudTop  ! Minimum cloud top [km] for selecting deep convection profiles
   integer, intent(in) :: lwi(:,:)

! !OUTPUT PARAMETERS:   
   real(kind=REAL64), intent(out) :: pNOx3D(i1:i2, j1:j2, k1:k2) ! Scaled production rate (no units conversion here)
...
! !INPUT PARAMETERS:
   real(kind=REAL64), allocatable, intent(in) :: lprslay(:,:,:)
   integer, intent(in)  :: imonth
   real(kind=REAL64),  intent(in)  :: dtrn(:,:,:)
   real(kind=REAL64),  intent(in)  :: mass(:,:,:)
   !      integer, intent(in)  :: cmi_flags(i1:i2, ju1:j2)
   real(kind=REAL64),  intent(in)  :: pnox2d(:,:)

! !OUTPUT PARAMETERS:
   real(kind=REAL64),  intent(inout)  :: pnox3d(:,:,:)
...
! !Local Variables:
   ...
   real(kind=REAL64) :: ppmtoppv                  ! conversion from parts per mass to parts per vol.
   
   real(kind=REAL64), allocatable :: ztop(:,:,:)  ! cloud layer tops
   real(kind=REAL64), allocatable :: htedge2(:)   ! Top edge heights after adjusting to match model-cloud
   real(kind=REAL64), allocatable :: fd0(:)       ! Used to calc cont of each "model-cloud"
   real(kind=REAL64), allocatable :: yout(:)      ! Layer to each CTM cloud layer
...
! !INPUT PARAMETERS:
      ...
      !     Total (CG+IC) flash rate (flashes per grid box per s)
      real(kind=REAL64), intent(in) :: flashrate(:,:)
...
! !OUTPUT PARAMETERS:
        ...
        real(kind=REAL64), intent(out) :: pnox(:,:)
