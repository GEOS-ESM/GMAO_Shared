...
!#ifdef GEOS
         !*** CCSM values
!         hfrazilmin = 0.10_dbl_kind ! min thickness of new frazil ice (m)
!#else
...
#ifdef GEOS
      integer, parameter :: my_task = 0,  master_task = 0
#endif
...
!#ifndef GEOS 
                  ...
                  write(nu_diag,*) my_task,':',i,j, &
                       'aicen(n) =', aicen(i,j,n)
                  write(nu_diag,*) my_task,':',i,j, &
                       'aicen(n+1) =', aicen(i,j,n+1)
                  write(nu_diag,*) my_task,':',i,j, &
                       'hicen_init(n) =', hicen_init(ij,n)
                  write(nu_diag,*) my_task,':',i,j, &
                       'hicen_init(n+1) =', hicen_init(ij,n+1)
                  write(nu_diag,*) my_task,':',i,j, &
                       'dhicen(n) =', dhicen(ij,n)
                  write(nu_diag,*) my_task,':',i,j, &
                       'dhicen(n+1) =', dhicen(ij,n+1)
                  write(nu_diag,*) my_task,':',i,j, &
                       'hin_max(n) =', hin_max(n)
                  write(nu_diag,*) my_task,':',i,j, &
                       'slope =', slope
#ifdef GEOS
                  write(nu_diag,*) 'lat = ', latd(1,1), &
                                   'lon = ', lond(1,1) 
#endif
!#endif
...
!
      ...
      qi0av = c0
      if(.not. form_fresh) then
         do ij = 1, icells
             i = indxi(ij)
             j = indxj(ij)
             do k = 1, nilyr
                 qi0(k) = -rhoi * (cp_ice*(Tmlt(k)-Tf(i,j))   &
                        + Lfresh*(1.-Tmlt(k)/Tf(i,j)) - cp_ocn*Tmlt(k)) 
                 qi0av  = qi0av + qi0(k)
             enddo
             qi0av = qi0av/rnilyr
...
!#ifdef GEOS
         ! following Schmidt et al.(2004) Sec. 3
         ! there is an extra term cp_ocn*Tf in the denominator Eq. 9 (BP) 
         ! this is the correct way to conserve energy and total enthalpy
!         vi0new(ij) = -fnew*dt / (rhoi*cp_ocn*Tf(i,j)+qi0av) ! note sign convention, qi < 0
!#else
...
!BOP
!
! !ROUTINE: freeboard_ccsm - snow-ice conversion
!
! !DESCRIPTION:
!
...
!
! NOTE: Mar/2010 B.Z. 
!       this subroutine is called after lateral_melt. the original 
!       call to freeboard in thickness_changes has to be commented out
!       in GEOS5 mode    
!
! !REVISION HISTORY:
!
...
!         Elizabeth C. Hunke, LANL
!
! modified: Mar/2010 by Bin Zhao to use it in GEOS5 
!
! !INTERFACE:
!
      subroutine freeboard_ccsm(nx_block,   ny_block,   &
                                ilo, ihi,   jlo, jhi,   &
                                dt,            & 
                                aicen,         & 
                                vicen,  vsnon, &
...
! !INPUT/OUTPUT PARAMETERS:
      ...
      real (kind=dbl_kind), dimension (nx_block,ny_block), &
         intent(inout) :: &
         snoice   , & ! concentration of ice
         fsalt        ! volume per unit area of ice          (m)
!EOP
!
      real (kind=dbl_kind), parameter :: &
!         hs_min = 1.e-4_dbl_kind  ! min snow thickness for computing Tsno (m)
         ! CCSM uses 0.00001
         hs_min = 1.e-5_dbl_kind  ! min snow thickness for computing Tsno (m)
...
!ocl novrec      !Fujitsu
      ...
      !-----------------------------------------------------------------
            ...
            if (dz > puny .and. hs > hs_min) then ! snow below freeboard
               jcells = jcells + 1
               indxi2(jcells) = i
               indxj2(jcells) = j
      ...
      !-----------------------------------------------------------------
               ...
               de(i,j) = esnon(i,j,n)*dhs/hs
               esnon(i,j,n) = esnon(i,j,n) - de(i,j)
               de(i,j) = de(i,j)/real(nilyr) ! energy going to each ice layer
      ...
      !-----------------------------------------------------------------
               ...
               hi = hi + dhi
               hs = hs - dhs
               vicen(i,j,n) = hi * aicen(i,j,n)
               vsnon(i,j,n) = hs * aicen(i,j,n)
      ...
      !-----------------------------------------------------------------
            ...
            ! history diagnostic
               snoice(i,j) = snoice(i,j) + dhi*aicen(i,j,n) 
...
!ocl novrec      !Fujitsu
            do ij = 1, jcells   ! just cells with freeboard adjustment
               ...
               j = indxj2(ij)
               eicen(i,j,ilyr1(n)+k-1) =  & 
                   eicen(i,j,ilyr1(n)+k-1) + de(i,j)
            enddo               ! ij
         enddo                  ! k

      enddo                     ! n

      end subroutine freeboard_ccsm
#endif
