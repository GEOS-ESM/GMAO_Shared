...
! 2014 MAT: Added allocate and deallocate routines.
!
! !INTERFACE:
!
      module ice_init
...
!
      ...
      logical, protected :: dyna_allocated   = .false.
      logical, protected :: dyna_deallocated = .false.
      logical, protected :: column_physics_allocated   = .false.
      logical, protected :: column_physics_deallocated = .false.
...
!BOP
!
! !IROUTINE: alloc_dyna_arrays - allocate ice_state variables
!
! !INTERFACE:
!
      subroutine alloc_dyna_arrays(root_pe, calling_routine)
!
! !DESCRIPTION:
!
...
!
      ...
      use ice_shortwave,        only: alloc_shortwave
      use ice_grid,             only: alloc_grid0
      use ice_work,             only: alloc_work
      use ice_flux,             only: alloc_flux
      use ice_forcing,          only: alloc_forcing
      use ice_history,          only: alloc_hist
      use ice_itd,              only: alloc_itd
      use ice_therm_vertical,   only: alloc_therm_vertical
      use ice_transport_driver, only: alloc_transport_driver
!
! !INPUT/OUTPUT PARAMETERS:
!
      logical, intent(in) :: root_pe
      character(len=*)    :: calling_routine
!
!EOP
!

      if (.not. dyna_allocated) then

         if (root_pe) write (*,*) "Allocating CICEDyna arrays from ", trim(calling_routine)
     ...
     end subroutine alloc_dyna_arrays
...
!BOP
!
! !IROUTINE: dealloc_dyna_arrays - deallocate ice_state variables
!
! !INTERFACE:
!
      subroutine dealloc_dyna_arrays(root_pe, calling_routine)
!
! !DESCRIPTION:
!
...
!
      ...
      use ice_grid,             only: dealloc_grid0
      use ice_work,             only: dealloc_work
      use ice_flux,             only: dealloc_flux
      use ice_forcing,          only: dealloc_forcing
      use ice_history,          only: dealloc_hist
      use ice_itd,              only: dealloc_itd
      use ice_therm_vertical,   only: dealloc_therm_vertical
      use ice_transport_driver, only: dealloc_transport_driver
!
!
! !INPUT/OUTPUT PARAMETERS:
!
...
!
      ...
      if (.not. dyna_deallocated) then

         if (root_pe) write (*,*) "Deallocating CICEDyna arrays from ", trim(calling_routine)
         ...
         call dealloc_transport_driver

         ! Flip the switch

         dyna_deallocated = .true.
         
      end if

     end subroutine dealloc_dyna_arrays
...
!BOP
!
! !IROUTINE: alloc_column_physics - allocate column physics variables
!
! !INTERFACE:
!
      subroutine alloc_column_physics(root_pe, calling_routine)
!
! !DESCRIPTION:
!
! Allocate column physics module variables
!
! !REVISION HISTORY:
!
! author: Yury Vikhliaev, NASA/GMAO
!
! !USES:
!
...
!
      ...
      if (.not. column_physics_allocated) then

         if (root_pe) write (*,*) "Allocating column physics arrays from ", trim(calling_routine)
         ...
         call alloc_therm_vertical

         ! tracers array

         allocate(trcr_depend(ntrcr), source=0_int_kind)

         ! Flip the switch

         column_physics_allocated = .true.
         
      end if

     end subroutine alloc_column_physics
...
!BOP
!
! !IROUTINE: dealloc_column_physics - deallocate column physics variables
!
! !INTERFACE:
!
      subroutine dealloc_column_physics(root_pe, calling_routine)
!
! !DESCRIPTION:
!
! Deallocate column physics module variables
!
! !REVISION HISTORY:
!
! author: Yury Vikhliaev, NASA/GMAO
...
!
      ...
      if (.not. column_physics_deallocated) then

         if (root_pe) write (*,*) "Deallocating column physics arrays from ", trim(calling_routine)
         ...
         call dealloc_therm_vertical

         ! Flip the switch
         
         deallocate(trcr_depend)
         
         column_physics_deallocated = .true.
         
      end if

     end subroutine dealloc_column_physics
...
!
#ifdef GEOS
      subroutine input_data(dtg, albiv, albsv, albii, albsi, &
                            cond, ustarm, ahm, ks, icesal, sp, dalb)
#else
...
!
      ...
      use ice_shortwave, only: albicev, albicei, albsnowv, albsnowi, &
                               dalb_mlt, snowpatch,    &
                               ahmax, shortwave, albedo_type, &
                               R_ice, R_pnd, R_snw
      use ice_atmo, only: atmbndy, calc_strair
      use ice_transport_driver, only: advection
#ifdef GEOS
      use ice_age, only: tr_iage, iage_converter, restart_age
#else
...
!
      ...
      namelist /ice_nml/ &
        ...
        ahmax,          ustar_min,       mu_rdg,                        & 
        R_ice,          R_pnd,           R_snw,         conduct,        &
      ...
      namelist /tracer_nml/   &
...
#ifdef GEOS
       real, intent(in) :: dtg, albiv, albsv, albii, albsi
       character (len=char_len), intent(in) :: cond
       real, intent(in) :: ustarm, ahm, ks, icesal, sp, dalb
       ...
       albicev       = albiv            ! visible ice albedo for h > ahmax
       albicei       = albii            ! near-ir ice albedo for h > ahmax
       albsnowv      = albsv            ! cold snow albedo, visible
       albsnowi      = albsi            ! cold snow albedo, near IR
       conduct       = trim(cond)       ! 'MU71' or 'bubbly' (Pringle et al 2007)
       ustar_min     = ustarm
       ahmax         = ahm 
       snowpatch     = real(sp, dbl_kind) 
       dalb_mlt      = real(dalb, dbl_kind) 
       tr_iage       = .true.           ! ice age
       update_ocn_f  = .true.          ! include fresh water and salt fluxes for frazil
       iage_converter= 1.0/(secday*days_per_year)
       nu_diag       = ice_stdout      ! default
       dt            = dtg
       calc_strair   = .true.    ! calculate wind stress
       ksno          = real(ks,dbl_kind)        ! snow conductivity 
       ice_ref_salinity  = real(icesal,dbl_kind) ! reference ice salinity for salt flux  
       return
#endif
