...
#ifdef GEOS
      logical(kind=log_kind), private     :: import_anglet = .false.
#endif
...
!
         ...
         allocate(toceanfr(nx_block,ny_block,max_blocks), source=1.0_dbl_kind)
         allocate(uoceanfr(nx_block,ny_block,max_blocks), source=1.0_dbl_kind)
...
!
      ...
      do iblk = 1, nblocks
         ...
         do j = jlo, jhi
         do i = ilo, ihi
           toceanfr(i,j,iblk) = real(frocean(i-nghost,j-nghost), kind=dbl_kind)
           !if(.not. tmask(i,j,iblk)) oceanfr(i,j,iblk) = c1
         enddo
         enddo
      enddo
      call ice_timer_start(timer_bound)
      call ice_HaloUpdate (toceanfr,           halo_info, &
                           field_loc_center,   field_type_scalar, &
                           fillValue=c1)
      call ice_timer_stop(timer_bound)
      call to_ugrid(toceanfr, uoceanfr)
      uoceanfr = min(c1, max(uoceanfr, c0)) 
#endif
#endif
...
! (7) ANGLE (radians)   
#ifdef GEOS
! below is from ACCESS-OM2 model
...
!
...
#ifdef GEOS
      call ice_read_global(nu_grid,12,work_g1,'rda8',.true., &
           err_occured = import_anglet)   ! ANGLET
      import_anglet = .not. import_anglet
      if(import_anglet) then 
      if (my_task == master_task) then
           write (nu_diag,*) '**Addional grid info is detected to be in &
                             the grid file**'
           write (nu_diag,*) '**it will be used instead of being &
                             recomputed**'
      endif      
      ...
      call ice_read_global(nu_grid,12,work_g1,'rda8',.true.)   ! ANGLET
      ...
      call ice_read_global(nu_grid,10,work_g1,'rda8',.true.)   ! TAREA
      call scatter_global(TAREA, work_g1, master_task, distrb_info, &
                          field_loc_center, field_type_scalar)

      call ice_read_global(nu_grid,11,work_g1,'rda8',.true.)   ! UAREA
...
#ifdef GEOS
      if(.not. import_anglet) then
      ! GEOS5 does NOT calculate TLON, TLAT. Instead, they are read in 
      !  from grid file (see routine popgrid)
      ! same as AusCOM  
#endif
