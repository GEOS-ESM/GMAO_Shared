...
!
      ...
      ! only for shortwave_ccsm3
      real (kind=dbl_kind) :: &
         ahmax           ! thickness above which ice albedo 
...
!BOP
!
! !ROUTINE: alloc_shortwave
!
! !DESCRIPTION:
!
!  Allocate ice_shortwave arrays
! 
...
!
         ...
         allocate(alvdrn(nx_block,ny_block,ncat,max_blocks), source=0.0_dbl_kind)
         allocate(alidrn(nx_block,ny_block,ncat,max_blocks), source=0.0_dbl_kind)
         allocate(alvdfn(nx_block,ny_block,ncat,max_blocks), source=0.0_dbl_kind)
         allocate(alidfn(nx_block,ny_block,ncat,max_blocks), source=0.0_dbl_kind)

         allocate(albicen(nx_block,ny_block,ncat,max_blocks), source=0.0_dbl_kind)
         allocate(albsnon(nx_block,ny_block,ncat,max_blocks), source=0.0_dbl_kind)
         allocate(albpndn(nx_block,ny_block,ncat,max_blocks), source=0.0_dbl_kind)

         allocate(Iswabsn(nx_block,ny_block,ntilyr,max_blocks), source=0.0_dbl_kind)

         allocate(Sswabsn(nx_block,ny_block,ntslyr,max_blocks), source=0.0_dbl_kind)

         allocate(fswsfcn(nx_block,ny_block,ncat,max_blocks), source=0.0_dbl_kind)
         allocate(fswthrun(nx_block,ny_block,ncat,max_blocks), source=0.0_dbl_kind)
         allocate(fswintn(nx_block,ny_block,ncat,max_blocks), source=0.0_dbl_kind)
...
!BOP
!
! !ROUTINE: dealloc_shortwave
!
! !DESCRIPTION:
!
!  Deallocate ice_shortwave arrays
! 
! !REVISION HISTORY: same as module
!
! !INTERFACE:
...
!#ifdef GEOS
            ! avoid negative albedos for melting snow
!            alvdfns(i,j) = max (alvdfns(i,j), albocn)
!            alidfns(i,j) = max (alidfns(i,j), albocn)
!#endif
         endif                  ! hs > puny
...
#ifdef GEOS
         druvrpen(i,j) = druvr(i,j) * (c1-alvdrni(i,j)) * (c1-asnow) * i0vis
         dfuvrpen(i,j) = dfuvr(i,j) * (c1-alvdfni(i,j)) * (c1-asnow) * i0vis
         drparpen(i,j) = drpar(i,j) * (c1-alvdrni(i,j)) * (c1-asnow) * i0vis  
         dfparpen(i,j) = dfpar(i,j) * (c1-alvdfni(i,j)) * (c1-asnow) * i0vis
#endif
...
!         fswpenidf = swidf(i,j) * (c1-alidfni(i,j)) * (c1-asnow) * i0nir  
        ...
        !                      'asnow = ', asnow 
        !    write(nu_diag, *) 'swabs = ', swabs, &
        !                      'fswpen = ', fswpen(i,j) 
        !    write(nu_diag, *) 'fswpenvdr = ', fswpenvdr, &
        !                      'fswpenvdf = ', fswpenvdf
        !    write(nu_diag, *) 'swabsv = ', swabsv, &
        !                      'swabsi = ', swabsi
        ! endif
#endif
...
#ifdef GEOS
         druvrthru(i,j) = druvrpen(i,j) * tranbot(i,j)
         dfuvrthru(i,j) = dfuvrpen(i,j) * tranbot(i,j) 
         drparthru(i,j) = drparpen(i,j) * tranbot(i,j)
         dfparthru(i,j) = dfparpen(i,j) * tranbot(i,j)
#endif
...
!-----------------------------------------------------------------------
      ...
      ! MAT Handle klev and klevp here and pass to dEdd codes
...
#ifdef GEOS
         !if(observe(1,1)) then
         !   write(nu_diag, *) 'A. aidrl = ',  aidrl(1,1), &
         !                      ' aidfl = ', aidfl(1,1)
         !   write(nu_diag, *) 'A. fnidr = ',  fnidr(1,1), &
         !                     'fi = ',fi(1,1) 
         !endif
#endif
!DIR$ CONCURRENT !Cray
...
#ifdef GEOS
         !if(observe(1,1)) then
         !   write(nu_diag, *) 'B. aidrl = ',  aidrl(1,1), &
         !                      ' aidfl = ', aidfl(1,1), &
         !                     'fs = ',fs(1,1) 
         !endif
#endif
...
#ifdef GEOS
         !if(observe(1,1)) then
         !   write(nu_diag, *) 'C. aidrl = ',  aidrl(1,1), &
         !                      ' aidfl = ', aidfl(1,1), &
         !                     'fp = ',fp(1,1) 
         !endif
#endif
...
#ifdef GEOS
        !if(observe(1,1)) then
        !  print*,'ns = ',ns, ' rupdir(0) = ', rupdir(0, 1)
        !  print*,'ns = ',ns, ' wghtns = ',wghtns(1,ns)
        !endif
#endif
        ...
        if( ns == 1) then      ! visible
          ...
          do ij = 1, icells_DE
            ...
            fthru(ij)  = fthru(ij) + &
               ...
               (fdifdn(klevp,ij)-fdifup(klevp,ij))*swvdf(i,j)
#ifdef GEOS
            druvrthru0(ij)  = druvrthru0(ij) + &
               (fdirdn(klevp,ij)-fdirup(klevp,ij))*druvr(i,j)
            dfuvrthru0(ij)  = dfuvrthru0(ij) + &
               (fdifdn(klevp,ij)-fdifup(klevp,ij))*dfuvr(i,j)
            drparthru0(ij)  = drparthru0(ij) + &
               (fdirdn(klevp,ij)-fdirup(klevp,ij))*drpar(i,j)
            dfparthru0(ij)  = dfparthru0(ij) + &
               (fdifdn(klevp,ij)-fdifup(klevp,ij))*dfpar(i,j)
#endif
...
#ifdef GEOS
         druvrthru(i,j) = druvrthru(i,j) + druvrthru0(ij)*fi(i,j)
         dfuvrthru(i,j) = dfuvrthru(i,j) + dfuvrthru0(ij)*fi(i,j)   
         drparthru(i,j) = drparthru(i,j) + drparthru0(ij)*fi(i,j) 
         dfparthru(i,j) = dfparthru(i,j) + dfparthru0(ij)*fi(i,j)
#endif
      enddo                     ! ij
...
! !INPUT/OUTPUT PARAMETERS:
      ...
      logical, dimension (nx_block,ny_block), &
         intent(in) :: &
         observe      ! cosine solar zenith angle
#endif
      real (kind=dbl_kind), dimension (nx_block,ny_block), &
         intent(in) :: &
         coszen      ! cosine solar zenith angle
...
#ifdef GEOS
         ! if(observe(1,1) .and. k == 1) then  
         !   print*, 'k = ', k, ue, extins 
         !   print*, 'k = ', k, 'rdif_a = ', rdif_a(k,ij)
         !   print*, 'k = ', k, ne, lm 
         !   print*, 'k = ', k, ts, g(k,ij)
         !   print*, 'k = ', k, tau(k,ij), w0(k,ij)
         !   print*, 'k = ', k, 'tdif_a = ', tdif_a(k,ij)
         ! endif
#endif
...
!#ifdef GEOS
!          if(observe(1,1) .and. k == 1) then  
!            print*, 'k = ', k, 'rdif_b = ', rdif_b(k,ij)
!            print*, 'k = ', k, 'tdif_b = ', tdif_b(k,ij)
!          endif
!#endif
...
#ifdef GEOS
      !    if(observe(1,1)) then  
      !      print*, 'k = ', klevp, rupdir(klevp,1), rupdif(klevp,1)
      !    endif
#endif
      do k=klev,0,-1
        do ij = 1, icells_DE
          ...
          rupdif(k,ij) = rdif_a(k,ij) + &
                          ...
                          refkp1*tdif_b(k,ij)
!#ifdef GEOS
!          if(observe(1,1) .and. k >= 2) then  
!            print*, 'k = ', k, rupdir(k,ij), rupdif(k,ij)
!            print*, 'k = ', k, rdir(k,ij), trnlay(k,ij) 
!            print*, 'k = ', k, tdir(k,ij),  refkp1 
!            print*, 'k = ', k, tdif_b(k,ij),rdif_b(k,ij)
!          endif
!#endif
        enddo       ! ij  
      enddo       ! k
