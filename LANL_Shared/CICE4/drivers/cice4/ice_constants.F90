...
#ifdef GEOS
         cp_ice    = real(MAPL_CAPICE,kind=dbl_kind),&! specific heat of fresh ice (J/kg/K)
#else
...
#ifdef GEOS
         ! use MAPL constants
         Tffresh   = real(MAPL_TICE,kind=dbl_kind),&! freezing temp of fresh ice (K)
         Lsub      = real(MAPL_ALHS,kind=dbl_kind),&! latent heat, sublimation freshwater (J/kg)
         Lvap      = real(MAPL_ALHL,kind=dbl_kind),&! latent heat, vaporization freshwater (J/kg)
#else
