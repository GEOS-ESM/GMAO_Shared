...
!                                  dxt      (:,:,iblk), dyt  (:,:,iblk),     &
    ...
    !-------------------------------------------------------------------
         ...
         do n = 1, ncat
            ...
            hiceflxe(:,:,n,iblk) = mtflxe(:,:,1,n)
            hiceflxn(:,:,n,iblk) = mtflxn(:,:,1,n)
            ...
            if (l_stop) then
               ...
               if (istop > 0 .and. jstop > 0)  then
                    write(nu_diag,*) 'Global i and j:',            &
                                     this_block%i_glob(istop),     &
                                     this_block%j_glob(jstop) 
                    if (istop > 1 .and. jstop > 1)  then
                      write(nu_diag,*) 'uvel(i,j)     = ', &
                                           uvel(istop,jstop,iblk)
                      write(nu_diag,*) 'uvel(i-1,j)   = ', &
                                           uvel(istop-1,jstop,iblk)
                      write(nu_diag,*) 'uvel(i,j-1)   = ', &
                                           uvel(istop,jstop-1,iblk)
                      write(nu_diag,*) 'uvel(i-1,j-1) = ', &
                                           uvel(istop-1,jstop-1,iblk)
                      write(nu_diag,*) 'vvel(i,j)     = ', &
                                           vvel(istop,jstop,iblk)
                      write(nu_diag,*) 'vvel(i-1,j)   = ', &
                                           vvel(istop-1,jstop,iblk)
                      write(nu_diag,*) 'vvel(i,j-1)   = ', &
                                           vvel(istop,jstop-1,iblk)
                      write(nu_diag,*) 'vvel(i-1,j-1) = ', &
                                           vvel(istop-1,jstop-1,iblk)
                      write(nu_diag,*) 'aicen(i,j)     = ', &
                                           mm(istop,jstop,n,iblk)
                      write(nu_diag,*) 'aicen(i+1,j)     = ', &
                                           mm(istop+1,jstop,n,iblk)
                      write(nu_diag,*) 'aicen(i-1,j)     = ', &
                                           mm(istop-1,jstop,n,iblk)
                      write(nu_diag,*) 'aicen(i,j+1)     = ', &
                                           mm(istop,jstop+1,n,iblk)
                      write(nu_diag,*) 'aicen(i,j-1)     = ', &
                                           mm(istop,jstop-1,n,iblk)
                      write(nu_diag,*) 'aicen(i+1,j+1)     = ', &
                                           mm(istop+1,jstop+1,n,iblk)
                      write(nu_diag,*) 'aicen(i-1,j-1)     = ', &
                                           mm(istop-1,jstop-1,n,iblk)
                      write(nu_diag,*) 'aicen(i-1,j+1)     = ', &
                                           mm(istop-1,jstop+1,n,iblk)
