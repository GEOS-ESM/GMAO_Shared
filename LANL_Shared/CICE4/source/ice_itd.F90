...
!
      ...
      integer (kind=int_kind), allocatable :: &
         ilyr1 (:), &    ! array position of top ice layer in each cat
         ilyrn (:), &    ! array position of bottom ice layer in each cat
         slyr1 (:), &    ! array position of top snow layer in each cat
         slyrn (:)       ! array position of bottom snow layer in each cat

      real (kind=dbl_kind), parameter :: &
#ifdef GEOS
         !*** CCSM values
         hi_min = p1     ! minimum ice thickness allowed (m)
#else
      ...
      character (len=35), allocatable :: c_hi_range(:)
...
!BOP
!
! !IROUTINE: alloc_itd - allocate ice_itd arrays
!
! !INTERFACE:
!
      subroutine alloc_itd
!
! !DESCRIPTION:
!
! Allocate itd arrays
...
!
         ...
         allocate(ilyr1 (ncat), source=0_int_kind)
         allocate(ilyrn (ncat), source=0_int_kind)
         allocate(slyr1 (ncat), source=0_int_kind)
         allocate(slyrn (ncat), source=0_int_kind)

         allocate(hin_max(0:ncat), source=0.0_dbl_kind)

         allocate(c_hi_range(ncat))

      end subroutine alloc_itd

!=======================================================================
!
!BOP
!
! !IROUTINE: dealloc_itd - deallocate ice_itd arrays
!
! !INTERFACE:
!
      subroutine dealloc_itd
!
! !DESCRIPTION:
!
! Deallocate itd arrays
!
! !REVISION HISTORY:
!
! author  Matthew A. Thompson, NASA/GMAO
...
!
         ...
         deallocate(c_hi_range)

      end subroutine dealloc_itd
...
!
...
#ifdef GEOS
      subroutine aggregate_ice_states
      ...
      end subroutine aggregate_ice_states
#endif
!=======================================================================
!BOP
!
...
!
...
#ifdef GEOS
      ! if limit_aice_in is present and false, these variables
      ! are not initialized. A BUG?
      dfresh    =  0.0 
      dfsalt    =  0.0
      dfhocn    =  0.0   
       if(present(punynum)) then
          mypuny = punynum
       else
          mypuny = 5.e-7_dbl_kind
       endif 
#endif
      ...
      if (limit_aice) then  ! check for aice out of bounds
         ...
         do i = ilo,ihi
#ifdef GEOS
            if (aice(i,j) > c1+mypuny .or. aice(i,j) < -mypuny) then
#else
      ...
      !-----------------------------------------------------------------
         ...
         do i = ilo, ihi
            if (aicen(i,j,n) < -puny) then
               ...
               return
!#ifdef GEOS
!            elseif ((aicen(i,j,n) >= -mypuny .and. aicen(i,j,n) < c0) .or. &
!                    (aicen(i,j,n) > c0 .and. aicen(i,j,n) <= mypuny)) then
!#else
...
!ocl novrec      !Fujitsu
      ...
      do i = ilo, ihi
         ...
         if (aice(i,j) > (c1+puny)) then
            ...
            return
#ifdef GEOS
         elseif (aice(i,j) > c1 .and. aice(i,j) < (c1+mypuny)) then
#else
