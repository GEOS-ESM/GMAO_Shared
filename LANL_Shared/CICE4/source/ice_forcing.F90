...
!
      ...
      character (char_len_long), allocatable, dimension(:) :: &  ! input data file names
        topmelt_file, &
        botmelt_file
...
!BOP
!
! !IROUTINE: alloc_forcing - allocate atmospheric forcing
!
! !INTERFACE:
!
...
!
        ...
        allocate(cldf(nx_block,ny_block,max_blocks), source=0.0_dbl_kind)

        allocate(fsw_data(nx_block,ny_block,2,max_blocks), source=0.0_dbl_kind)
        allocate(cldf_data(nx_block,ny_block,2,max_blocks), source=0.0_dbl_kind)
        allocate(fsnow_data(nx_block,ny_block,2,max_blocks), source=0.0_dbl_kind)
        allocate(Tair_data(nx_block,ny_block,2,max_blocks), source=0.0_dbl_kind)
        allocate(uatm_data(nx_block,ny_block,2,max_blocks), source=0.0_dbl_kind)
        allocate(vatm_data(nx_block,ny_block,2,max_blocks), source=0.0_dbl_kind)
        allocate(wind_data(nx_block,ny_block,2,max_blocks), source=0.0_dbl_kind)
        allocate(strax_data(nx_block,ny_block,2,max_blocks), source=0.0_dbl_kind)
        allocate(stray_data(nx_block,ny_block,2,max_blocks), source=0.0_dbl_kind)
        allocate(Qa_data(nx_block,ny_block,2,max_blocks), source=0.0_dbl_kind)
        allocate(rhoa_data(nx_block,ny_block,2,max_blocks), source=0.0_dbl_kind)
        allocate(potT_data(nx_block,ny_block,2,max_blocks), source=0.0_dbl_kind)
        allocate(zlvl_data(nx_block,ny_block,2,max_blocks), source=0.0_dbl_kind)
        allocate(flw_data(nx_block,ny_block,2,max_blocks), source=0.0_dbl_kind)
        allocate(sst_data(nx_block,ny_block,2,max_blocks), source=0.0_dbl_kind)
        allocate(sss_data(nx_block,ny_block,2,max_blocks), source=0.0_dbl_kind)
        allocate(uocn_data(nx_block,ny_block,2,max_blocks), source=0.0_dbl_kind)
        allocate(vocn_data(nx_block,ny_block,2,max_blocks), source=0.0_dbl_kind)
        allocate(sublim_data(nx_block,ny_block,2,max_blocks), source=0.0_dbl_kind)
        allocate(frain_data(nx_block,ny_block,2,max_blocks), source=0.0_dbl_kind)

        allocate(topmelt_data(nx_block,ny_block,2,max_blocks,ncat), source=0.0_dbl_kind)
        allocate(botmelt_data(nx_block,ny_block,2,max_blocks,ncat), source=0.0_dbl_kind)

        allocate(ocn_frc_m(nx_block,ny_block,max_blocks,nfld,12), source=0.0_dbl_kind)

        allocate(topmelt_file(ncat))
        allocate(botmelt_file(ncat))
...
!BOP
!
! !IROUTINE: dealloc_forcing - deallocate atmospheric forcing
!
! !INTERFACE:
!
...
!
        ...
        deallocate(fsw_data)
        deallocate(cldf_data)
        deallocate(fsnow_data)
        deallocate(Tair_data)
        deallocate(uatm_data)
        deallocate(vatm_data)
        deallocate(wind_data)
        deallocate(strax_data)
        deallocate(stray_data)
        deallocate(Qa_data)
        deallocate(rhoa_data)
        deallocate(potT_data)
        deallocate(zlvl_data)
        deallocate(flw_data)
        deallocate(sst_data)
        deallocate(sss_data)
        deallocate(uocn_data)
        deallocate(vocn_data)
        deallocate(sublim_data)
        deallocate(frain_data)

        deallocate(topmelt_data)
        deallocate(botmelt_data)

        deallocate(ocn_frc_m)

        deallocate(topmelt_file)
        deallocate(botmelt_file)
