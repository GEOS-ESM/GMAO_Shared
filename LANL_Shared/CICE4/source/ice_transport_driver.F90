...
!BOP
!
! !IROUTINE: alloc_transport_driver - allocate ice_transport_driver arrays
!
! !INTERFACE:
!
      subroutine alloc_transport_driver
!
! !DESCRIPTION:
!
! Allocate transport driver arrays
!
! !REVISION HISTORY:
!
...
!
         ...
         ntrace = 2+ntrcr+nilyr+nslyr
                          
         allocate(tracer_type(ntrace), source=0)
         allocate(depend(ntrace), source=0)

         allocate(has_dependents(ntrace))


      end subroutine alloc_transport_driver

!=======================================================================
!
!BOP
!
! !IROUTINE: dealloc_transport_driver - deallocate ice_transport_driver arrays
!
! !INTERFACE:
!
      subroutine dealloc_transport_driver
!
! !DESCRIPTION:
!
! Deallocate transport driver arrays
!
! !REVISION HISTORY:
!
! author  Matthew A. Thompson, NASA/GMAO
...
!
         ...
         deallocate(tracer_type)
         deallocate(depend)

         deallocate(has_dependents)

      end subroutine dealloc_transport_driver
...
!
#ifdef GEOS
      subroutine init_transport(m_advection)
#else
...
!lipscomb - two branches for now; consolidate later
      ...
      if (my_task == master_task) then
         write(nu_diag,*) ' USE Incremental Remapping for transport'
      endif
#endif
...
#ifdef GEOS
      if (my_task == master_task) then
         write(nu_diag,*) ' USE UpWind for transport'
      endif
#endif
         continue
...
! !INTERFACE:
!
      subroutine transport_upwind (dt, narr)
!
! !DESCRIPTION:
!
...
!
      ...
      use ice_state
      use ice_exit
      use ice_grid, only: HTE, HTN, tarea, toceanfr
      use ice_flux, only: hiceflxe, hiceflxn
      use ice_calendar, only: istep1
      use ice_timers
!
! !INPUT/OUTPUT PARAMETERS:
...
!
      ...
      real (kind=dbl_kind), dimension(0:2*ncat) ::     &
         asum_init      ,&! initial global ice area
         asum_final       ! final global ice area
      ...
      works(:,:,:,:) = c0  
      !*** properly init these since the ghost cells are not
      !*** filled for the southern most row
      uee(:,:,:)     = c0
      vnn(:,:,:)     = c0

#if 0
      asum_init(0) = global_sum(aice0(:,:,:),     distrb_info,       &
                             field_loc_center, tarea)
      do n = 1, ncat
          asum_init(n) = global_sum(aicen(:,:,n,:),     distrb_info,       &
                                field_loc_center, tarea)
      enddo
      do n = ncat+1, 2*ncat
          asum_init(n) = global_sum(vicen(:,:,n,:),     distrb_info,       &
                                field_loc_center, tarea)
      enddo
#endif
    ...
    !-------------------------------------------------------------------
      ...
      do iblk = 1, nblocks
         ...
         do i = 1, ihi
            ...
            b = vnn(i,j,iblk)
            do n=1,ncat
              hiceflxe(i,j,n,iblk) = p5*HTE(i,j,iblk)*((a+abs(a))*vicen(i,j,n,iblk) &
                                                      +(a-abs(a))*vicen(i+1,j,n,iblk)) 
              hiceflxn(i,j,n,iblk) = p5*HTN(i,j,iblk)*((b+abs(b))*vicen(i,j,n,iblk) &
                                                      +(b-abs(b))*vicen(i,j+1,n,iblk))
            enddo
         enddo
         enddo
      enddo
...
#if 0
      asum_final(0) = global_sum(aice0(:,:,:),     distrb_info,       &
                             field_loc_center, tarea)
      do n = 1, ncat
          asum_final(n) = global_sum(aicen(:,:,n,:),     distrb_info,       &
                                field_loc_center, tarea)
      enddo
      do n = ncat+1, 2*ncat
          asum_final(n) = global_sum(vicen(:,:,n,:),     distrb_info,       &
                                field_loc_center, tarea)
      enddo
         ...
         if (my_task == master_task) then
            ...
            if (l_stop) then
               write (nu_diag,*) 'istep1, my_task, iblk =',     &
                                  istep1, my_task, iblk
               write (nu_diag,*) 'transport: conservation error, cat 0'
               !call abort_ice('ice upwind transport: conservation error')
            endif
            ...
            do n = 1, 2*ncat               
               ...
               if (l_stop) then
                  ...
                  write (nu_diag,*) 'transport: conservation error, cat ',n
                !  call abort_ice     &
                 !      ('ice upwind transport: conservation error')
               endif
            enddo               ! n
...
!
      ...
      real (kind=dbl_kind), dimension(nx_block,ny_block), intent(in) :: &
         ...
         toceanfr             ! fractional ocean area of the grid cell
!
!EOP
!
      integer (kind=int_kind) ::     &
    ...
    !-------------------------------------------------------------------
      ...
      do n = 1, narrays
         ...
         do i = 1, ihi
            worka(i,j)=     &
              !toceanfr(i,j)*toceanfr(i+1,j)*upwind(phi(i,j,n),phi(i+1,j,n),uee(i,j),HTE(i,j))
              upwind(phi(i,j,n),phi(i+1,j,n),uee(i,j),HTE(i,j))
            workb(i,j)=     &
              !toceanfr(i,j)*toceanfr(i,j+1)*upwind(phi(i,j,n),phi(i,j+1,n),vnn(i,j),HTN(i,j))
