...
!     ---------------
!     real  zlev0(10)
!     data  zlev0/ 1000.0, 850.0, 700.0, 500.0, 400.0, 300.0, 250.0, 200.0, 150.0, 100.0 / 
...
! **********************************************************************
      ...
      interface
         subroutine init_levs( fname,lm,lev )
         implicit none
         integer  lm
         character*256    fname
         real, pointer :: lev(:)
      ...
      if( nargs.eq.0 ) then
          ...
          do n=1,nargs
            ...
            if( trim(arg(n)).eq.'-levs' ) then
                 nl = 1
                 read(arg(n+nl),fmt='(a1)') char
                 do while (char.ne.'-' .and. n+nl.ne.nargs )
                 nl = nl+1
                 read(arg(n+nl),fmt='(a1)') char
                 enddo
                 if( char.eq.'-' ) nl = nl-1
...
!     ---------------------
      if( nl.eq.-999 ) call init_levs( fname(1),nl,zlev )
      print *, 'Verifying Levels:'
      print *, '----------------'
      write(6,*) ( zlev(n),n=1,nl )
      print *
...
! ----------------------------------
      ...
      if( n2d.ne.0 ) then
          ...
          do k=1,size(fields_2d(n)%alias)
          print *, '  ALIASES: ',trim(fields_2d(n)%alias(k))
          enddo
          print *
          enddo
      endif
      if( n3d.ne.0 ) then
          ...
          do k=1,size(fields_3d(n)%alias)
          print *, '  ALIASES: ',trim(fields_3d(n)%alias(k))
          enddo
          print *
          enddo
      endif
...
! Find Analysis and Climatology for Forecast Time: nymd,nhms
! ----------------------------------------------------------
...
! Read Systematic Error File and Modify Forecast Fields
! -----------------------------------------------------
        if( syserr ) then
...
! Determine beginning and ending i&j for Region
! ---------------------------------------------
      call bounds (lat1,lat2,lon1,lon2, &
                   jbeg,jend,ibeg,iend,im,jm)


! Loop over Fields and Levels
! ---------------------------
...
! Define Deviations from Area Means
! ---------------------------------
      do j=jbeg,jend
      do i=ibeg,iend
      if( defined(f(i,j),fundef)   .and.  &
...
! Subtract Climatology Deviations and Compute Variances
! -----------------------------------------------------
      do j=jbeg,jend
      do i=ibeg,iend
...
! Compute Regional Mean Variances and Anomaly Correlation
! -------------------------------------------------------
        c1 = 0
        c2 = 0
        c3 = 0
      ...
      if( n3d.gt.0 .and. nfield.gt.n2d ) then
          ...
          if( trim(fields_3d(n)%name).eq.'q' .and. iregion.eq.3 .and. zlev(lev).eq.925 ) then
              write(6,1006) int(925),nymd,nhms,hour,rms(iregion,lev,nfield,nt,1),fmean,amean,cmean
              printout = .true.
          endif
           if( trim(fields_3d(n)%name).eq.'q' .and. iregion.eq.3 .and. zlev(lev).eq.850 ) then
              write(6,1006) int(850),nymd,nhms,hour,rms(iregion,lev,nfield,nt,1),fmean,amean,cmean
              printout = .true.
          endif
           if( trim(fields_3d(n)%name).eq.'q' .and. iregion.eq.3 .and. zlev(lev).eq.700 ) then
              write(6,1006) int(700),nymd,nhms,hour,rms(iregion,lev,nfield,nt,1),fmean,amean,cmean
              printout = .true.
          endif
          if( trim(fields_3d(n)%name).eq.'t' .and. iregion.eq.3 .and. zlev(lev).eq.925 ) then
              write(6,1005) int(925),nymd,nhms,hour,rms(iregion,lev,nfield,nt,1),fmean,amean,cmean
              printout = .true.
          endif
           ...
           if( trim(fields_3d(n)%name).eq.'t' .and. iregion.eq.3 .and. zlev(lev).eq.700 ) then
              write(6,1005) int(700),nymd,nhms,hour,rms(iregion,lev,nfield,nt,1),fmean,amean,cmean
              printout = .true.
          endif
...
! Write out correlation and rms data
! ----------------------------------
      year   =     nymdb/10000
...
! Write out correlation tabl ctl1 (uniform times)
! -----------------------------------------------
...
! Write out correlation tabl ctl2 (actual times)
! ----------------------------------------------
...
! Write out global data tabl ctl3 (actual times)
! ----------------------------------------------
 ...
 1005 format(1x,i4,'-mb TR Temp   at ',i8,2x,i6.6,' (',i3,' hrs)  rmse: ',f9.7, &
             3x,'fcst: ',f7.2,2x,'ana: ',f7.2,2x,'cli: ',f7.2,' (m)')
 1006 format(1x,i4,'-mb TR SpHum  at ',i8,2x,i6.6,' (',i3,' hrs)  rmse: ',f9.7, &
             3x,'fcst: ',f7.2,2x,'ana: ',f7.2,2x,'cli: ',f7.2,' (m)')
...
!*********************************************************************
!****         Open Climatology DataSet and Initialization         ****
!*********************************************************************
...
! Initial Forecast Fields to ZERO
! -------------------------------
      do n=1,n2d
...
! Determine Climatology TOD Dataset
! ---------------------------------
      if( num.eq.1 ) then
          nid = id(num)
          ncl =    num
...
!     write(6,100) imonm,facm,imonp,facp
!100  format(1x,'Interpolating Climatology for Month ',i2,
!    .          ' (',f5.3,') and Month ',i2,' (',f5.3,')')
...
!*********************************************************************
!****           READ DATA SET TWICE TO GET BOTH MONTHS            ****
!*********************************************************************
...
! Read All ANA Files to Gather Date and Time Information
! ------------------------------------------------------
      if( first ) then
...
! Initial Analysis Fields to NOT Found
! ------------------------------------
      do n=1,n2d
      found_2d(n) = .false.
      enddo
...
! Read Appropriate ANA Files to Get Data
! --------------------------------------
      do num=1,ndates
...
! Set Analysis Fields to UNDEF if NOT Found
! -----------------------------------------
      do n=1,n2d
      ...
      character*256  contact
      character*256  levunits
      character*256, pointer ::  vname(:)
      character*256, pointer :: vtitle(:)
      character*256, pointer :: vunits(:)
...
! Read Forecast File to Gather Lev Information
! --------------------------------------------

         call gfio_open       ( fname,1,id,rc )
         call gfio_diminquire ( id,im,jm,lm,ntime,nvars,ngatts,rc )
          ...
          call gfio_close(id,rc)
          deallocate ( lon,lat,yymmdd,hhmmss,vname,vtitle,vunits,kmvar,vrange,prange )
...
! Initial Forecast Fields to NOT Found
! ------------------------------------
      do n=1,n2d
      found_2d(n) = .false.
      enddo
...
! Loop Through All Dates within Forecast Files
! --------------------------------------------
      do num=1,ndates
      if( dates(1,num).eq.nymd .and. dates(2,num).eq.nhms ) then
...
! READ Forecast Fields from Forecast Files
! ----------------------------------------
      do n=1,nvars
...
! Set Forecast Fields to UNDEF if NOT Found
! -----------------------------------------
      do n=1,n2d
