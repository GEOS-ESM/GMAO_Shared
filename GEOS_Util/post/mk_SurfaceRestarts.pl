...
set mk_catchANDcnRestars_X   = ( $Bin/mk_catchANDcnRestarts.x )

set OUT_til   = `ls OutData/\*.til | cut -d '/' -f2`
set IN_til    = `ls InData/\*.til | cut -d '/' -f2`

if (\$catchFLG) then
  set model catch
  set IN_rst   = `ls InData/\*catch_* | cut -d '/' -f2`
  set OUT_rst  = catch_internal_rst
endif

if (\$catchcnFLG) then
  set model catchcn$cnlist[0]
  set IN_rst    = `ls InData/\*catchcn_* | cut -d '/' -f2`
  set OUT_rst  = catchcn$cnlist[0]_internal_rst
endif

set params = ( -model \$model -time \$rsttime -in_tile_file \$IN_til )
set params = ( \$params -out_bcs OutData/clsm -out_tile_file \$OUT_til -out_dir ./ )
set params = ( \$params -surflay \$surflay -in_wemin \$weminIN  -out_wemin \$weminOUT )
set params = ( \$params -in_rst \$IN_rst -out_rst \$OUT_rst )
