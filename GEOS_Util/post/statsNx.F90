...
!     ------------
      ...
      if( trim(outfile) /= "NULL" ) then
          ...
          if(failed) then
             ...
             print* ,'        -verif VERIFICATION (e.g., ecmwf)'
             print* ,' Aborting ...'
             call exit(1)
          endif
          gmaopy=.true.
...
! -----------------------------------------
   ...
   10 continue
      if( id.eq.0 ) then
          ...
          if( rc.ne.0 ) then
              print *, 'Systec Error File: ',trim(efile),' NOT found!'
              call ESMF_Finalize()
              stop
          endif
          call gfio_diminquire ( id,im,jm,lm,ntime,nvars,ngatts,rc )
...
!     ---------------------------------------
 ...
 100  format(/,1x,'no find sErr for ',i8,2x,i6.6,'  ZEROs will be used',/)
