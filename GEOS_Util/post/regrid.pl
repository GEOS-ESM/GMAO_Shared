...
sub get_fvrst {
    ...
    $id = $1 if $fvrstname =~ m/(\S+)\.fvcore_internal_rst/;
    $id = "" unless $id;
...
sub check_rst_files {
    ...
    # turn off saltwater, if openwater and seaicethermo restarts are found
    #---------------------------------------------------------------------
    if ($SURFACE{"openwater_internal_rst"}) {
        if ($SURFACE{"seaicethermo_internal_rst"}) {
            $SURFACE{"saltwater_internal_rst"} = 0;
        }
    }
    if ($SURFACE{"saltwater_internal_rst"}) {
        $SURFACE{"openwater_internal_rst"} = 0;
        $SURFACE{"seaicethermo_internal_rst"} = 0;
    }
...
sub regrid_surface_rsts {
    ...
    # *another* link to clsm below that was in
    # a previous version
    if ($mk_catchcn) {
    ...
    #--------------------------------------------------------------
    split_saltwater() if $SURFACE{"saltwater_internal_rst"};
    ...
    if ($H1{"rescale"} and ($mk_catch or $mk_catchcn)) {
        ...
        # Note this was done above for if catchcn is being
        # regridded. It is done here because it used to be done
