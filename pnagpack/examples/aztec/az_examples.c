...

{
  ...
  extern void add_row_3D(int row, int i, double val[], int bindx[]);

  total_nz = N_update*avg_nonzeros_per_row + 1;
  *bindx   = (int *) calloc(total_nz, sizeof(int));
  *val     = (double *) calloc(total_nz, sizeof(double));
  ...
  for (i = 0; i < N_update; i++) {
    add_row_3D(update[i], i, *val, *bindx);
...
void add_row_5pt(int row, int location, double val[], int bindx[])
...

{
  ...
  bindx[k] = row + NP;   if ((row/NP)%n !=     n-1) val[k++] = -1.0;
  bindx[k] = row - NP;   if ((row/NP)%n !=       0) val[k++] = -1.0;
  bindx[k] = row + n*NP; if ((row/(NP*n))%n != n-1) val[k++] = -1.0;
  bindx[k] = row - n*NP; if ((row/(NP*n))%n !=   0) val[k++] = -1.0;
...
void add_row_3D(int row, int location, double val[], int bindx[])
...

{
  ...
  bindx[k] = row + n*NP;   if ((row/(NP*n))%n  != n-1) val[k++] = -1.0;
  bindx[k] = row - n*NP;   if ((row/(NP*n))%n  != 0  ) val[k++] = -1.0;
  bindx[k] = row + n*n*NP; if (bindx[k]    < n*n*n*NP) val[k++] = -1.0;
  bindx[k] = row - n*n*NP; if (bindx[k]          >= 0) val[k++] = -1.0;
...

{
  ...
  for (stride = 1; stride <= n; stride = stride*n) {
    ...
    if ((i != 0) && (i != n-1)) val[location] += 30.0/12.0;
...

{
  ...
  *indx  = (int *)    calloc(total_blks+1, sizeof(int));
  *bindx = (int *)    calloc(total_blks+1, sizeof(int));
  *rnptr = (int *)    calloc(num_update_blks+1, sizeof(int));
  *bnptr = (int *)    calloc(num_update_blks+1, sizeof(int));
  *val   = (double *) calloc(total_nz, sizeof(double));
...

{
  ...
  extern void setup_Ke(double Ke[3][3], double x1, double y1, double x2,
                       double y2, double x3, double y3);
  extern void read_coordinates(double **x, double **y, int update_indx[], int,
                               int);
  extern void add_to_element(int row, int column, double element, double val[],
                             int bindx[], int diag);
...

{
  ...
  T[0] = (int *) calloc(N_triangle, sizeof(int));
  T[1] = (int *) calloc(N_triangle, sizeof(int));
  T[2] = (int *) calloc(N_triangle, sizeof(int));
...

{
  ...
  *val   = (double *) malloc((N_points*MAX_NZ_ROW+1)*sizeof(double));
  *bindx = (int    *) malloc((N_points*MAX_NZ_ROW+1)*sizeof(int));
...
void setup_Ke(double Ke[3][3], double x1, double y1, double x2, double y2,
              double x3, double y3)
...

{
  ...
  Ke[0][0] = (y3-y2)*(y3-y2) + (x3-x2)*(x3-x2);
  Ke[0][1] = (y3-y2)*(y1-y3) + (x3-x2)*(x1-x3);
  Ke[0][2] = (y2-y1)*(y3-y2) + (x2-x1)*(x3-x2);

  Ke[1][0] = (y3-y2)*(y1-y3) + (x3-x2)*(x1-x3);
  Ke[1][1] = (y3-y1)*(y3-y1) + (x3-x1)*(x3-x1);
  Ke[1][2] = (y1-y3)*(y2-y1) + (x1-x3)*(x2-x1);
  Ke[2][0] = (y2-y1)*(y3-y2) + (x2-x1)*(x3-x2);
  Ke[2][1] = (y1-y3)*(y2-y1) + (x1-x3)*(x2-x1);
  Ke[2][2] = (y2-y1)*(y2-y1) + (x2-x1)*(x2-x1);
...

{
  ...
  *x   = (double *) calloc((total+1), sizeof(double));
  *y   = (double *) calloc((total+1), sizeof(double));
