...
C
      SUBROUTINE LANSO(N,LANMAX,astore,MAXPRS,ENDL,ENDR,J,NEIG,RITZ,BND,
...
C
      CALL STPONE(N,astore,R,WRK,ALF,NQ,MSGLVL)
      J = 1
      IF (N.EQ.1) THEN
         NEIG = 1
         RITZ(1) = ALF(1)
...
C
            IF (BET(J).EQ.ZERO) THEN
               RNM = STARTV(N,astore,J,R,WRK,NQ,EPS,MSGLVL)
               ENOUGH = RNM.EQ.ZERO
               IF (ENOUGH) GOTO 15
            ENDIF
C
...
C
            CALL PURGE(N,astore,LL,J,R,R(NQ(1)),R(NQ(4)),R(NQ(3)),WRK,
